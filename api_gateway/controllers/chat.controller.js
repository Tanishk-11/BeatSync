...
export const handleChat = async (req, res) => {
  try {
    // 1. Extract the user's question and the chat history from the request body.
    const { question, history } = req.body;

    // Validate that a question is present.
    if (!question) {
    ...
    // 3. Forward the question AND the history to the Python chatbot service.
    // ====================================================================================
    // FIX: The chatbot service requires both the 'question' and the 'history'
    // to maintain conversational context. We now pass the history along.
    // ====================================================================================
    const response = await axios.post(chatbotServiceUrl, {
      question: question,
      history: history || [], // Pass the history, or an empty array if it's the first message
    });
