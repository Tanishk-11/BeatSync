...
# class TSM(tf.keras.layers.Layer):
#     # <<< MODIFIED: ADDED __init__ TO ACCEPT n_frame ARGUMENT
#     def __init__(self, n_frame=10, **kwargs):
#         super(TSM, self).__init__(**kwargs)
#         self.n_frame = n_frame

#     # <<< MODIFIED: REMOVED n_frame FROM call() SIGNATURE
#     def call(self, x, fold_div=3):
#         nt, h, w, c = x.shape
#         # <<< MODIFIED: USING self.n_frame
...
#         padding_1 = padding_1[:, -1, :, :, :]
#         padding_1 = tf.expand_dims(padding_1, 1)
#         _, out1 = tf.split(out1, [1, self.n_frame - 1], axis=1) # <<< MODIFIED
#         out1 = tf.concat([out1, padding_1], axis=1)
...
#         padding_2 = padding_2[:, 0, :, :, :]
#         padding_2 = tf.expand_dims(padding_2, 1)
#         out2, _ = tf.split(out2, [self.n_frame - 1, 1], axis=1) # <<< MODIFIED
#         out2 = tf.concat([padding_2, out2], axis=1)
...
# def TSM_Cov2D(x, n_frame, nb_filters=128, kernel_size=(3, 3), activation='tanh', padding='same'):
#     # <<< MODIFIED: CHANGED HOW TSM LAYER IS CALLED
...
# import tensorflow as tf
# from tensorflow.python.keras import backend as K
# from tensorflow.python.keras.layers import Conv2D, Input, AveragePooling2D, multiply, Dense, Dropout, Flatten, Reshape
# from tensorflow.python.keras.models import Model
